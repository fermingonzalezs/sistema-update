...
// Estilos profesionales mejorados para el PDF - Basado en ReciboPDF
const styles = StyleSheet.create({
    ...
    // Columnas de la tabla (4 columnas - Sin Serial)
    colItem: {
        width: '60%',
        textAlign: 'center',
        borderRightWidth: 0.5,
    ...
    // Sección de condiciones/observaciones
...
const PresupuestoDocument = ({ data }) => {
    ...
    const calcularVencimiento = () => {
        ...
        const fechaEmision = new Date(data.invoice.rawDate || Date.now());
        // Sumar vigencia (horas)
        const fechaVencimiento = new Date(fechaEmision.getTime() + (data.vigenciaHoras * 60 * 60 * 1000));
            ...
            <Page size="A4" style={styles.page}>
                ...
                <View style={styles.receiptHeader}>
                    ...
                    <View style={styles.clientHeaderSection}>
                        <Text style={styles.clientHeaderLabel}>PRESUPUESTO PARA:</Text>
                    ...
                    <View style={styles.documentTitleSection}>
                        ...
                        <Text style={styles.vigenciaText}>
                            Válido hasta: {calcularVencimiento()}
                        </Text>
                    </View>
                </View>
                ...
                <View style={styles.totalsSection} wrap={false}>
                    <View style={styles.totalsContainer}>
                        ...
                        <View style={[styles.totalsHeader, { borderBottomWidth: 0 }]}>
                            <Text style={styles.totalsHeaderText}>Total Final</Text>
                            <Text style={[styles.totalsHeaderText, { borderRightWidth: 0 }]}>
                ...
                {(data.condiciones || data.observaciones) && (
                    <View style={styles.observacionesSection} wrap={false}>
                        {data.condiciones && (
                            <View style={{ marginBottom: data.observaciones ? 8 : 0 }}>
                                <Text style={styles.observacionesLabel}>CONDICIONES COMERCIALES:</Text>
                                <Text style={styles.observacionesText}>{data.condiciones}</Text>
                            </View>
                        )}
                ...
                <View style={styles.thankYouSection} wrap={false}>
                    <Text style={styles.thankYouText}>QUEDAMOS A DISPOSICIÓN</Text>
                </View>
...
// Función para convertir datos al formato del documento
export const convertirPresupuestoADocumento = (presupuesto) => {
    return {
        company: {
        ...
        invoice: {
            number: presupuesto.numero_recibo,
            rawDate: presupuesto.fecha, // Guardamos fecha cruda para cálculos
            date: new Date(presupuesto.fecha).toLocaleDateString('es-AR', {
                day: '2-digit',
                month: '2-digit',
                year: 'numeric'
            })
        ...
        client: {
            name: presupuesto.cliente_nombre || 'Cliente No Especificado',
            address: presupuesto.cliente_direccion || '',
...
export const generarYDescargarPresupuesto = async (presupuesto) => {
    try {
        const presupuestoData = convertirPresupuestoADocumento(presupuesto);
        const blob = await pdf(<PresupuestoDocument data={presupuestoData} />).toBlob();
        ...
        return {
            success: true,
            message: 'Presupuesto abierto en nueva pestaña'
        };
    } catch (error) {
