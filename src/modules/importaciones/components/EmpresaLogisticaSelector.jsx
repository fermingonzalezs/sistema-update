...
import { useEmpresasLogistica } from '../hooks/useEmpresasLogistica';

const EmpresaLogisticaSelector = ({ value, onChange, className = '', placeholder = 'Seleccionar empresa...' }) => {
    const { empresas, loading, addEmpresa } = useEmpresasLogistica();
    ...
    useEffect(() => {
        const handleClickOutside = (event) => {
            if (wrapperRef.current && !wrapperRef.current.contains(event.target)) {
                const dropdown = document.getElementById('empresa-logistica-selector-dropdown');
    ...
    const handleAddEmpresa = async (e) => {
        ...
        const result = await addEmpresa(newEmpresa.trim());
        if (result.success) {
            ...
            alert('Error al agregar empresa: ' + result.error);
        }
    };

    const dropdownContent = (
        <div
            id="empresa-logistica-selector-dropdown"
        ...
        >
            {!isAdding ? (
                <>
                    ...
                    <div className="py-1">
                        {loading ? (
                            ...
                            <div className="px-4 py-2 text-sm text-slate-500">No se encontraron empresas</div>
                        ) : (
                            filteredEmpresas.map((empresa) => (
                                <div
                                    key={empresa.id}
                                    className={`px-4 py-2 text-sm cursor-pointer hover:bg-slate-100 flex items-center justify-between ${value === empresa.nombre ? 'bg-emerald-50 text-emerald-700' : 'text-slate-700'}`}
                                    onClick={() => handleSelect(empresa.nombre)}
                                >
                                    <span>{empresa.nombre}</span>
                                    {value === empresa.nombre && <Check className="w-4 h-4" />}
                                </div>
                            ))
                        )}
                    </div>
                ...
                <div className="p-3">
                    <h4 className="text-sm font-medium text-slate-700 mb-2">Nueva Empresa</h4>
                    <div className="flex space-x-2">
                        <input
                            ...
                            autoFocus
                            onClick={(e) => e.stopPropagation()}
                            onKeyDown={(e) => { if (e.key === 'Enter') handleAddEmpresa(e); }}
                        />
                        <button
...
export default EmpresaLogisticaSelector;
