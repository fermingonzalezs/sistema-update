...
// Estilos unificados con recibos y garantías
const styles = StyleSheet.create({
  ...
  // Header de la empresa (unificado)
  companyHeader: {
    flexDirection: 'row',
  ...
  // Header del documento (unificado)
  ...
  // Información del cliente y equipo (unificado)
  clientSection: {
    flexDirection: 'row',
    marginBottom: 18,
    gap: 25,
  ...
  // Tabla de servicios y repuestos (unificado)
  table: {
    marginBottom: 15,
    borderWidth: 1,
    borderColor: '#E5E7EB',
  ...
  // Sección de condiciones (unificado)
  conditionsSection: {
    marginBottom: 8,
    backgroundColor: '#F9FAFB',
    padding: 8,
          ...
          {data.repuestos && data.repuestos.map((repuesto, index) => (
            ...
            >
              <View style={[styles.tableCellDescription, styles.colDescription]}>
                ...
                {repuesto.esTercero && (
                  <Text style={{ fontSize: 6, color: '#F59E0B', marginTop: 2 }}>
                    (Repuesto de tercero)
                  </Text>
                )}
              </View>
...
// Función para abrir presupuesto en navegador
export const abrirPresupuestoPDF = async (reparacion, presupuesto) => {
  try {
    console.log('🔄 Generando presupuesto de reparación...');
    ...
    if (nuevaPestana) {
      console.log('✅ Presupuesto abierto en nueva pestaña');
      ...
      return { 
        ...
        filename: `presupuesto-${presupuestoData.numeroPresupuesto}-${reparacion.cliente_nombre || 'cliente'}-${new Date().toISOString().split('T')[0]}.pdf`
      };
    } else {
